...
metadata {
			definition (name: "OpenGarage.io Handler2", namespace: "littlegumSmartHome", author: "Ian Lindsay") {
	...
	tiles (scale: 2){
		standardTile("garagedoor", "device.garagedoor", width: 6, height: 3) {
        ...
        valueTile("dist", "device.dist", decoration: "flat", inactiveLabel: false, width: 5, height: 1) {
        	state "val", label:'Distance ${currentValue} cm', defaultState: true
		}

        valueTile("rssi", "device.rssi", decoration: "flat", inactiveLabel: false, width: 5, height: 1) {
        	state "val", label:'rssi: ${currentValue} dB', defaultState: true
		}

        valueTile("fwv", "device.fwv", decoration: "flat", inactiveLabel: false, width: 5, height: 1) {
        	state "val", label:'Version ${currentValue}', defaultState: true
		}

		main("garagedoor")
		details(["garagedoor", "refresh", "dist", "rssi", "fwv", "cid"])
	}
    simulator {
        // simulator metadata
    }
...
def poll()
{
	log.debug "Poll - Refreshing Values "
    api("getstatus", [])
}
...
def parse(description) {
    ...
    if(json.mac){
        ...
        if(json.fwv){
        	resFwv = createEvent(name: "fwv", value: json.fwv)
        }
        if(json.cid){
        	resCid = createEvent(name: "cid", value: json.cid)
        }
        if(json.rssi){
        	resRssi = createEvent(name: "rssi", value: json.rssi)
        }
    }
    
    log.debug "after state.doorStatus: $state.doorStatus"
    
    return [result, resDist, resFwv, resCid, resRssi]
}
