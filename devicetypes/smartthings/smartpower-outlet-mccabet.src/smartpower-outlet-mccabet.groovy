...
metadata {
	...
	definition(name: "SmartPower Outlet-mccabet", namespace: "smartthings", author: "SmartThings", ocfDeviceType: "oic.d.smartplug") {
		capability "Actuator"
		capability "Switch"
...
// TJM 09-24-2017	add maxPower Tile			
...
//		details(["switch", "refresh"])
        details(["switch","voltage","amperage","power","energy","energykVAh","powerfactor","blank2x","refresh","blank2x"])	}
}
def maxpower = 0
...
def parse(String description) {
	...
	if (event) {
		if (event.name == "power") {
			...
			def value = (event.value as Integer) / 10
			event = createEvent(name: event.name, value: value, descriptionText: '{{ device.displayName }} energy is {{ value }} KWh', translatable: true)
